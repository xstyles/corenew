          ...
          self.areRequiredPopulated = function() {
            ...
            $(inputs).each(function() {
              ...
              if (input.attr('type') == 'radio') {
                ...
                if (document.querySelector(`input[name=${name}]:checked`).value > 0) {
                  fullCount++
                } else {
                  return false
                }
    ...
    foreach ($form['fields'] as &$field) {
      ...
      if ($field->fields) {
        foreach ($field->fields as &$nestedField) {
          $input_ids = false;
          $index = $nestedField->get_context_property('itemIndex');

          switch (GFFormsModel::get_input_type($nestedField)) {
            ...
            case 'radio':
              foreach ($nestedField->choices as $choiceIndex => $choice) {
                $input_ids[] = $choiceIndex;
              }
              break;

            default:
              $html_ids[] = "#input_{$form['id']}_{$nestedField['id']}-{$index}";
              break;
          }
          ...
          foreach ($input_ids as $input_id) {
            $html_ids[] = "#input_{$form['id']}_{$nestedField['id']}_{$input_id}-{$index}";
          }
        }
      } else {
        if (!$field['isRequired']) {
